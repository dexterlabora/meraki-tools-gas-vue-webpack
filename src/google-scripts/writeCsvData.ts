...
export function writeCsvData(csvContent, title, location: string) {
  try { // fix the bug on Utilities.parseCsv() google script function which does not allow newlines in csv strings @simonjamain
    ...
    switch (location) {
        case "overwrite": toActiveCellOverwrite(csvData);
            break;

        case "newSheet": toNewSheet(csvData);
            break;

        case "newRows": toNewRows(csvData);
            break;
...
function toNewRows(csvData) { // const sheet = SpreadsheetApp.getActiveSheet();
...
function toActiveCellOverwrite(csvData) { // const sheet = SpreadsheetApp.getActiveSheet();
    sheet().getRange(sheet().getActiveCell().getLastRow(), sheet().getActiveCell().getColumn(), csvData.length, csvData[0].length).setValues(csvData);
}
// to use still
